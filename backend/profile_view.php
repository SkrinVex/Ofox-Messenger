    ...
    if ($user_id !== null && isset($profile_data['user_id'])) {
        ...
        if ($current_user) {
            ...
            if ($friendship['count'] > 0) {
                ...
                $stmt = $conn->prepare("SELECT id, created_at FROM friend_requests WHERE sender_id = ? AND receiver_id = ? AND status = 'pending'");
                $stmt->execute([$current_user_id, $target_user_id]);
                ...
                if ($sent_request) {
                    ...
                    $profile_data['friend_request_id'] = $received_request['id'];
                    $profile_data['friend_request_date'] = $received_request['created_at'];
                } else {
